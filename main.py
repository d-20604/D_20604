...
# [중요] 1. 네 번째 그래프의 기반이 되는 'df_daily'를 만드는 과정입니다.
# (만약 원본 데이터프레임 이름이 'df'가 아니라면, df 부분을 사용 중인 변수명으로 변경해 주세요)
# 예: 날짜별 전체 관객수 합계 구하기
if 'df_daily' not in locals() and 'df_daily' not in globals():
  # 만약 df_daily가 아직 없다면, 원본 df에서 생성한다고 가정
  # (실제 본인의 원본 데이터프레임명과 컬럼명에 맞게 수정 필요)
  # 예시: df_daily = df.groupby('기준일자')['관객수'].sum()
  pass

# 혹시 네 번째 그래프 코드에서 다른 이름으로 저장되어 있다면 아래에서 맞춰주세요.
# 예: df_daily = 네번째_데이터프레임_변수명

# 2. df_daily가 Series 형태라면 DataFrame으로 안전하게 변환
if isinstance(df_daily, pd.Series):
  df_daily = df_daily.reset_index()

# 3. DataFrame이 되었는데 '기준일자'가 컬럼에 없다면 인덱스 리셋 시도
if '기준일자' not in df_daily.columns:
  df_daily = df_daily.reset_index()

# 4. 기준일자를 datetime 형식으로 변환 후 연-월(YYYY-MM) 단위 추출
df_daily['기준일자'] = pd.to_datetime(df_daily['기준일자'])
df_daily['연월'] = df_daily['기준일자'].dt.to_period('M')

# 5. 관객수 관련 컬럼 자동 탐지 (전체관객수, 관객수 등)
target_col = None
